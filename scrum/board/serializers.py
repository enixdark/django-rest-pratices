...
class TaskSerializer(serializers.ModelSerializer):
    ...
    def validate_sprints(self,attrs,source):
        ...
        if slef.object and self.object.pk:
            if sprint != self.object.sprint:
                if self.object.status == task.STATUS_DONE:
                    mgs = _('cannot change the sprint of a completed task')
                    raise serializers.ValidationError(mgs)
                if sprint and sprint.end < date.today():
                    mgs = _('cannot not assign tasks to past sprints')
                    raise serializers.ValidationError(mgs)
            else:
                if sprint and sprint.end < date.today():
                    mgs = _('cannot add task to the past sprints')
                    raise serializers.ValidationError(mgs)
            return attrs
